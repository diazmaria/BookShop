...
{
    os << CIAN "Carrito de la compra de " << u.id() << NORMAL " [Artículos: " << u.n_articulos() << "]"<< endl << " Cant." << '\t'
    << "Artículo" << endl << setfill('=') << setw(66) << "" << endl;

    for (Usuario::Articulos::const_iterator i = u.compra().begin();
         i != u.compra().end(); i++) {

        os << "   " << i->second << '\t';

        imp_prologo(os, *i->first);
        imp_epilogo(os, *i->first);
