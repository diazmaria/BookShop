...
void Pedido_Articulo::mostrarDetallePedidos(ostream& os) const
{
    ...
    {
        os << CIAN "Pedido núm. " << i->first->numero() << endl
        << NORMAL "Cliente: " << i->first->tarjeta()->titular()->id()
...
void Pedido_Articulo::mostrarVentasArticulos(ostream& os) const
{
	...
	{
        os << ROSA "Ventas de [" << i->first->referencia() << "] \"" << i->first->titulo()
        << "\"" NORMAL <<  " [Pedidos: "<< i->second.size() << "]" << endl
        << setfill('=') << setw(66) << "" << endl
